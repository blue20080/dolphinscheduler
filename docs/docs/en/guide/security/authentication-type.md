...
2. Set the redirect URI to `http://{your-etl-host:port}/etl/login/oauth2/code/{registrationId}`.
...
> **Note:** When a user clicks a provider's login button, they are first directed to a specific endpoint on the ETL backend (e.g., `/etl/oauth2/authorization/keycloak`). The backend then constructs the full request and redirects the user's browser to the OIDC provider's login page.
