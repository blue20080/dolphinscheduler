...
> NOTE: If you use MySQL, you need to manually download [mysql-connector-java driver][mysql] (8.0.16) and move it to the ETL module libs directories,
...
ETL stores metadata in a relational database. Currently, PostgreSQL and MySQL are supported. The following sections initialize each database type.

> If you use MySQL, manually download [mysql-connector-java driver][mysql] (8.0.16) and move it to `api-server/libs`, `alert-server/libs`, `master-server/libs`, `worker-server/libs`, and `tools/libs`.
...
After creating the ETL database, run the shell script to initialize its schema:
...
Some datasources are natively supported by ETL, while others require users to download JDBC drivers manually.
For licensing reasons, the ETL distribution cannot include drivers for MySQL, Oracle, SQL Server, and some other databases.
...
restarting `api-server` and `worker-server`.

> Note: If you only want to use MySQL in the datasource center, there is no requirement for the version of MySQL JDBC driver.
> To use MySQL as the ETL metadata database, use [version 8.0.16 or later](https://repo1.maven.org/maven2/mysql/mysql-connector-java/8.0.16/mysql-connector-java-8.0.16.jar).
