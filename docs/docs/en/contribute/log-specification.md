...
For example, to enable `DEBUG` logging for the `org.springframework.web` package in the `standalone` module, add the following configuration to `etl-dev-SNAPSHOT-bin/standalone-server/conf/logback-spring.xml`:
